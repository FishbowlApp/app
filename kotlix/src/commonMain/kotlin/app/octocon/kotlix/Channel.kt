...
class Channel(
  ...
  ) = tryEmit(event.value, payload, ref, joinRef, payloadText)
  ...
  internal fun tryEmit(
    ...
    joinRef: String? = null,
    payloadText: String? = null
  ) = tryEmit(Message(joinRef, ref, topic, event, payload, payloadText))
    ...
    scope.launch(CoroutineName("CHANNEL_RESPONSE_LISTENER_$topic")) {
      this@Channel.collect { message ->
        when (message.event) {
          ...
          Event.REPLY.value -> {
            tryEmit(replyEventName(message.ref), message.rawPayload, message.ref, message.joinRef, message.payloadText)
          }
